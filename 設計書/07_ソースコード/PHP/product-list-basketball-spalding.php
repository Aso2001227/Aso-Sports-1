...
<div class="product">
    ...
    try{
        $pdo=getDB();
        $sql=$pdo->prepare('select image,item_name,price,category_id from m_items where item_code IN(130,131,132,133)');
        $sql->execute();
        ...
        foreach ($resultList as $item) {
            ...
            echo '</form>';
            echo '<form action="cart.php?item_name=',$item_array[$num],'" method="post">';
            echo '</div>';
            $num++;
        }
